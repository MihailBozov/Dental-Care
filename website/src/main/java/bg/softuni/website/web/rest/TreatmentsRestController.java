package bg.softuni.website.web.rest;
...
import bg.softuni.website.services.rest.TreatmentRestService;
...
public class TreatmentsRestController {
    ...
    @Autowired
    public TreatmentsRestController(TreatmentRestService treatmentRestService) {
        this.treatmentRestService = treatmentRestService;
    }
    
    @ModelAttribute("allTreatments")
    public List<TreatmentDto> initTreatmentDto() {
        return this.treatmentRestService.getAllTreatmentDtos();
    }
...
//    --------------------------------------------------------------------
    ...
    public String treatment(@Valid NewTreatmentDto newTreatmentDto,
        ...
        this.treatmentRestService.newTreatment(newTreatmentDto);
    ...
    public String deleteTreatment(@PathVariable Long id) throws IOException {
        ...
        try {
            boolean isDeleted = this.treatmentRestService.deleteTreatment(id);
            if (isDeleted) {
                return "redirect:/treatments?success=true";
            }
            else {
    ...
    public EditTreatmentDto editTreatment(@PathVariable Long id) {
        return this.treatmentRestService.getEditTreatmentDto(id);
    }
    ...
    public String editTreatment(@PathVariable Long id,
        ...
        boolean isEdited = this.treatmentRestService.editTreatment(editTreatmentDto, id);
        if (isEdited) {
            return "redirect:/treatments?success=true";
        }
        return "redirect:/treatments?success=false";
